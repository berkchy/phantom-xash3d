...
public:
	...
	static inline HIMAGE SPR_Load( const char *szPicName )
	{
		return engfuncs.pfnSPR_Load( szPicName );
	}

	static inline int SPR_Frames( HIMAGE hPic )
	{
		return engfuncs.pfnSPR_Frames( hPic );
	}

	static inline int SPR_Width( HIMAGE hPic, int frame )
	{
		return engfuncs.pfnSPR_Width( hPic, frame );
	}

	static inline int SPR_Height( HIMAGE hPic, int frame )
	{
		return engfuncs.pfnSPR_Height( hPic, frame );
	}

	static inline void SPR_Set( HIMAGE hPic, int r, int g, int b, int a = 255 )
	{
		engfuncs.pfnSPR_Set( hPic, r, g, b, a );
	}

	static inline void SPR_Draw( int frame, int x, int y, const wrect_t *prc = NULL )
	{
		engfuncs.pfnSPR_Draw( frame, x, y, prc );
	}

	static inline void SPR_DrawHoles( int frame, int x, int y, const wrect_t *prc = NULL )
	{
		engfuncs.pfnSPR_DrawHoles( frame, x, y, prc );
	}

	static inline void SPR_DrawTrans( int frame, int x, int y, const wrect_t *prc = NULL )
	{
		engfuncs.pfnSPR_DrawTrans( frame, x, y, prc );
	}

	static inline void SPR_DrawAdditive( int frame, int x, int y, const wrect_t *prc = NULL )
	{
		engfuncs.pfnSPR_DrawAdditive( frame, x, y, prc );
	}

	static inline void SPR_DrawAdditiveScale( int frame, int x, int y, const wrect_t *prc, float scale )
	{
		engfuncs.pfnSPR_DrawAdditiveScale( frame, x, y, prc, scale );
	}
